...
export const ChatArea: React.FC<ChatAreaProps> = ({
  ...
  return (
    <main className="flex-1 flex flex-col h-screen bg-white dark:bg-slate-950 relative transition-colors">
      {/* Top Navigation Bar */}
      <header className="h-14 border-b border-slate-200/80 dark:border-slate-800 px-6 flex items-center justify-between bg-white dark:bg-slate-950 z-10">
        <div className="flex items-center gap-3">
          <div className="text-xs font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
            <span className="text-slate-400 dark:text-slate-500">Context:</span>
            {selectedDoc ? (
              <span className="inline-flex items-center gap-1 text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/40 px-2 py-0.5 rounded border border-blue-100 dark:border-blue-900">
                📄 {selectedDoc.filename}
              </span>
            ) : (
              <span className="inline-flex items-center gap-1 text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded">
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-8 py-6 space-y-6">
        {messages.length === 0 ? (
          ...
          messages.map((msg) => (
            ...
            >
              {msg.role === 'assistant' && (
                <div className="w-8 h-8 rounded-lg bg-blue-600 dark:bg-blue-500 text-white flex items-center justify-center shrink-0 shadow-sm shadow-blue-500/20">
              ...
              >
                ...
                {msg.role === 'assistant' && msg.detected_language && (
                  <div className="flex items-center gap-2 mb-2 pb-1.5 border-b border-slate-100 dark:border-slate-800 text-[10px] text-slate-400 dark:text-slate-500 font-mono">
                    <span>Query detected: <b className="text-slate-600 dark:text-slate-300 uppercase">{msg.detected_language}</b></span>
                    <span>•</span>
                    <span>Answer: <b className="text-blue-600 dark:text-blue-400 uppercase">{msg.target_language || 'Auto'}</b></span>
                  </div>
                )}
              ...
              {msg.role === 'user' && (
                <div className="w-8 h-8 rounded-lg bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 flex items-center justify-center shrink-0 font-bold text-xs">
        ...
        {isLoading && (
          <div className="flex gap-3 max-w-3xl mx-auto justify-start">
            ...
            <div className="rounded-2xl px-4 py-3 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
      ...
      <div className="p-4 border-t border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-950">
        <div className="max-w-3xl mx-auto relative flex items-end rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/60 focus-within:bg-white dark:focus-within:bg-slate-900 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100 dark:focus-within:ring-blue-950/40 transition-all p-2">
          <textarea
            ref={textareaRef}
            rows={1}
            value={input}
            ...
            className="flex-1 bg-transparent border-0 resize-none text-xs text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none max-h-32 p-1.5 leading-normal"
          />
          <button
        ...
        <p className="text-center text-[11px] text-slate-400 dark:text-slate-500 mt-2">
          DOC-Lingo grounds responses strictly on retrieved document chunks.
        </p>
      </div>
    </main>
  );
